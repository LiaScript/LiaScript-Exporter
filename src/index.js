  ...
  app.ports.output.subscribe(function (event) {
    ...
    switch(format) {
      ...
      case "scorm1.2": {
        ...
        temp.mkdir('scorm', function(err, tmp) {

          console.warn(tmp);

          let tempPath = path.join(tmp, "pro")
          ...
          const config = {
            ...
            package: {
              ...
              name: output,
              author: json.definition.author,
              outputFolder: path.join(tmp, 'scorm_packages'),
              description: json.comment,
              //keywords: ['scorm', 'test', 'course'],
              //typicalDuration: 'PT0H5M0S',
              //rights: `©${new Date().getFullYear()} My Amazing Company. All right reserved.`,
          ...
          fs.copy(
            ...
            .then(() => {
              console.warn("copy assets ... done");

              let key = argv.k || argv.key

              let index = fs.readFileSync(path.join(tempPath, 'index.html'), 'utf8')
              ...
              fs.writeFile(path.join(tempPath, 'index.html'), index, function (err) {
                if (err) {
                  console.error(err)
                  return
                }
